...
def dropwave_fcn(x):
    return (-(1 + cos(12 * sqrt(x[:,0]**2 + x[:,1]**2))) 
            / (0.5 * (x[:,0]**2 + x[:,1]**2) + 2))
...
OptimizationObject.POPULATION_SIZE = 200
OptimizationObject.RESULTS_BASE_DIR = results_dir
OptimizationObject.CMA_FITNESS_FUNCTION = dropwave_fcn
OptimizationObject.CMA_INITIAL_STEP_SIZE = (OptimizationObject.UPPER_BOUND - 0) / np.sqrt(OptimizationObject.CHROMOSOME_LENGTH)
...
def levi_fcn(x):
    return (sin(3 * np.pi * x[:,0])**2 + 
            (x[:,0] - 1)**2 * (1 + sin(3 * np.pi * x[:,1])**2) + 
            (x[:,1] - 1)**2 * (1 + sin(2 * np.pi * x[:,1])**2))
...
OptimizationObject.POPULATION_SIZE = 200
OptimizationObject.RESULTS_BASE_DIR = results_dir
OptimizationObject.CMA_FITNESS_FUNCTION = levi_fcn
OptimizationObject.CMA_INITIAL_STEP_SIZE = (OptimizationObject.UPPER_BOUND - 0) / np.sqrt(OptimizationObject.CHROMOSOME_LENGTH)
